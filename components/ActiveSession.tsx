...
import { Timer, Check, Plus, Trash2, Video, History, Scale, TrendingUp, ChevronLeft } from 'lucide-react';
...
export const ActiveSession: React.FC<ActiveSessionProps> = ({ routine, onFinish, onBack, existingSession }) => {
      ...
      <div className="sticky top-0 bg-white/95 dark:bg-dark-bg/95 backdrop-blur z-20 px-4 py-3 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-gray-500 font-mono">{formatTime(session.durationSeconds)}</div>
          </div>
        </div>
                    ...
                    <div key={set.id} className={`grid grid-cols-12 gap-2 items-center transition-all ${set.completed ? 'opacity-50' : ''}`}>
                        ...
                        <div className="col-span-3 flex justify-center gap-1">
                            <button 
                                ...
                                className={`w-full h-10 rounded-md flex items-center justify-center transition-colors ${
                                    set.completed 
